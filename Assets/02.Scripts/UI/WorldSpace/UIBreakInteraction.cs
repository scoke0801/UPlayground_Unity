...
namespace UPlayGround.UI
{
    ...
    /// 위치 추적·카메라 뒤 처리는 UIActorHpBar / UIDangerRing 패턴을 그대로 따른다.
    /// Screen Space Canvas(UIWorldSpaceHudLayer) 아래에 부착된다.
    /// </summary>
    public class UIBreakInteraction : MonoBehaviour, IActorBreakInteractionView
    {
        [Header("Anchor")]
        ...
        public bool ManagedLateTick(float deltaTime, float unscaledTime)
        {
            ...
            // 타겟 소멸 시 자가 정리 (UIActorHpBar / UIDangerRing 패턴)
            if (_target == null)
            {
                Release();
                return false;
        ...
        private void UpdatePosition()
        {
            ...
            // 카메라 뒤면 숨김 (SetActive 토글 대신 alpha — UIActorHpBar / UIDangerRing과 동일)
            bool behindCamera = screenPos.z < 0f;
            _canvasGroup.alpha = behindCamera ? 0f : 1f;
            if (behindCamera) return;

            // UI_HUD_InteractionKey와 같은 방식으로 화면 좌표를 직접 반영한다.
