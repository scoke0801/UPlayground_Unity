...
namespace UPlayGround.UI
{
    ...
    ///
    /// 위치 추적·카메라 뒤 처리는 UIActorHpBar 패턴을 따른다.
    /// 진행은 UIWorldSpaceHudLayer의 일괄 LateUpdate에서 스케일 시간을 전달받으므로
    /// 히트스톱/일시정지(Time.timeScale 변화) 시 자동으로 함께 멈춘다.
    /// Screen Space - Overlay Canvas(UIWorldSpaceHudLayer) 아래에 부착된다.
    /// </summary>
    public class UIDangerRing : MonoBehaviour, IActorDangerRingView
    {
        [Header("Ring")]
        ...
        private UIWorldSpaceHudLayer _owner;
        ...
        /// <summary>
        /// 링을 부착할 앵커 소켓 해석. 명시적 UIDangerRing 소켓이 최우선,
        ...
        private Transform ResolveAnchorSocket(GameActor actor)
        {
            ...
            Transform socket = actor.GetSocket(ActorSocketType.UIDangerRing);
        ...
        public bool ManagedLateTick(float deltaTime, float unscaledTime)
        {
            ...
            // 타겟 소멸 시 자가 정리 (UIActorHpBar와 동일)
            if (_target == null)
            {
                Release();
                return false;
        ...
        private void UpdatePosition()
        {
            ...
            // 카메라 뒤면 숨김 (SetActive 토글 대신 alpha — UIActorHpBar와 동일)
