...
// InputCallbackKey를 값 비교가 가능하도록 readonly struct로 변경 (권장)
    private readonly struct InputCallbackKey : IEquatable<InputCallbackKey>
    {
        ...
        public bool Equals(InputCallbackKey other) => ActionMapName == other.ActionMapName && ActionName == other.ActionName;
    ...
    private class InputCallbackData
    {
        ...
        public InputCallbackData(Action<InputAction.CallbackContext> callback, Func<bool> checkFunc, Action cancelCallback, InputLayer layer)
        {
            Callback = callback;
            CheckFunc = checkFunc;
