    ...
    public sealed class MotionEventAddPopup : PopupWindowContent
    {
        ...
        static class MotionEventMetadata
        {
            ...
            public static List<EventMeta> GetAll()
            {
                ...
                var result = new List<EventMeta>();
                foreach (Type type in MotionEventTypeRegistry.GetAllEventTypes())
                {
                    ...
                    string categoryName = descriptor?.Category ?? "기타";
                    if (!categories.TryGetValue(categoryName, out EventCategory category))
                    {
                    ...
                    result.Add(new EventMeta(
                        ...
                        descriptor?.Aliases?.Concat(new[] { type.Name }).ToArray()
                            ?? new[] { type.Name }));
                }
            ...
            public static List<EventPreset> GetPresets()
            {
                Dictionary<string, Type> types =
                    MotionEventTypeRegistry.GetAllEventTypes()
                        .GroupBy(type => type.Name)
                        .ToDictionary(group => group.Key, group => group.First());
                var result = new List<EventPreset>();

                if (HasTypes(types, "BeginCollisionEvent", "ComboWindowEvent"))
                    result.Add(new EventPreset(
                        "melee_basic",
                        "근접 공격 기본",
                        "Collision, ComboWindow를 기본 타이밍으로 추가합니다.",
                        start => new[]
                        {
                            CreateTimed(types["BeginCollisionEvent"], start + 0.10f, 0.15f),
                            CreateTimed(types["ComboWindowEvent"], start + 0.25f, 0.25f),
                        },
                        "melee", "attack", "combo", "근접", "공격", "콤보"));

                if (HasTypes(types, "CameraEffectEvent", "BeginCollisionEvent", "FinishAttackEvent"))
                    result.Add(new EventPreset(
                        "camera_melee",
                        "카메라 연출 공격",
                        ...
                        start => new[]
                        {
                            CreateTimed(types["CameraEffectEvent"], start, 0.25f),
                            CreateTimed(types["BeginCollisionEvent"], start + 0.12f, 0.15f),
                            CreateTimed(types["FinishAttackEvent"], start + 0.45f, 0.05f),
                        },
                        "camera", "shake", "attack", "카메라", "연출"));

                if (HasTypes(types, "CameraEffectEvent", "SpecialBreakAttackEvent"))
                    result.Add(new EventPreset(
                        ...
                        {
                            CreateTimed(types["CameraEffectEvent"], start, 0.25f),
                            CreateTimed(types["SpecialBreakAttackEvent"], start + 0.18f, 0.05f),
                        },
                        "break", "special", "finish", "브레이크", "특수공격", "처형"));

                if (HasTypes(types, "SlashVFXEvent"))
                    result.Add(new EventPreset(
                        "slash_vfx_basic",
                        "Slash VFX 기본",
                        ...
                        start => new[]
                        {
                            CreateTimed(types["SlashVFXEvent"], start, 0.05f),
                        },
                        "slash", "weapon", "blade", "vfx", "검기", "참격"));

                if (HasTypes(types, "PlaySoundEvent", "SpawnProjectileEvent"))
                    result.Add(new EventPreset(
                        "projectile_basic",
                        "투사체 발사 기본",
                        ...
                        start => new[]
                        {
                            CreateTimed(types["PlaySoundEvent"], start, 0.05f),
                            CreateTimed(types["SpawnProjectileEvent"], start + 0.10f, 0.05f),
                        },
            ...
            static bool HasTypes(
                IReadOnlyDictionary<string, Type> types,
                params string[] names) =>
                names.All(types.ContainsKey);

            static MotionEventBase CreateTimed(
                Type type,
                float start,
                float duration)
            {
                MotionEventBase motionEvent =
                    MotionEventTypeRegistry.CreateEventInstance(type);
                ApplyDefaultTime(motionEvent, start, duration);
                return motionEvent;
            }
