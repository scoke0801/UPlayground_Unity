...
- `UI_Scene_CharacterSelect`에서 기존 무기 효과 영역을 제거하고 대표 패시브 2행만 노출하도록 구현 완료.
...
- `UI_Scene_CharacterSelect`는 패시브를 조회·표시하지만 런타임 효과를 직접 적용하지 않는다.
...
| P-03 | `UI_Scene_CharacterSelect`와 런타임은 같은 `CharacterPassiveDatabaseSO`를 읽는다. UI용 설명을 별도로 복제하지 않는다. |
| P-04 | `UI_Scene_CharacterSelect`는 표시와 캐릭터 확정만 담당한다. 패시브 Grant나 Effect 적용은 하지 않는다. |
...
| P-13 | 캐릭터당 런타임 패시브 개수는 제한하지 않는다. `UI_Scene_CharacterSelect`에는 세트에서 명시한 대표 패시브 중 최대 2개만 표시한다. |
...
| 캐릭터 선택 UI | `CharacterSelectDatabaseSO`, `UI_Scene_CharacterSelect` | 같은 캐릭터 타입으로 패시브 표시 |
...
CharacterPassiveDatabaseSO
        ...
        ├──────────────────────────────┐
        ▼                              ▼
UI_Scene_CharacterSelect              PassiveAbilityController
...
public sealed class CharacterPassiveSetSO : ScriptableObject
{
    ...
    [Tooltip("UI_Scene_CharacterSelect에 표시할 대표 패시브. passives에 포함된 항목만, 최대 2개.")]
...
- `UI_Scene_CharacterSelect._passiveDatabase`: 선택 화면 표시용.
...
## 8. `UI_Scene_CharacterSelect` 표시와 처리

### 8.1 책임

`UI_Scene_CharacterSelect`의 책임:
...
- 최대 표시 개수는 `UI_Scene_CharacterSelect` 상수 `MaxRepresentativePassiveCount = 2`로 고정한다.
...
12. `UI_Scene_CharacterSelect`와 `PartyConfigSO`가 같은 Passive Database를 참조하는지.
...
2. `PartyConfigSO`와 `UI_Scene_CharacterSelect`에 Database 참조 추가.
...
| `UI/Scene/CharacterSelect/UI_Scene_CharacterSelect.cs` | 패시브 상세 목록 표시 |
...
- 캐릭터당 패시브 총개수는 무제한.
- `UI_Scene_CharacterSelect`에는 명시적으로 지정한 대표 패시브 최대 2개만 표시.
