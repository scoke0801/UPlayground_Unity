...
일반론은 피하고, 1인 개발 + 스타일리시 액션 게임 컨텍스트에 바로 적용 가능한 것만 정리한다.
...
- **우리에게 의미:** BT만으로는 그룹 의도 표현 한계가 분명하다는 강한 증거. 1인 개발 규모에 HTN 도입은 과투자. 그룹 의도 표현은 BT 외부에 별도 컨트롤러를 두는 현재 구조 유지가 합리
