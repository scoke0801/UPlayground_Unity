...
namespace UPlayGround.UI.Party.EditorTools
{
    ...
    /// - 3열(보유 동료 / 출전 파티 / 상세) + 하단 바 구성. 배틀 슬롯 4개는 인라인 인스턴스.
    /// - 스킬 4종·궁극기 게이지는 데이터 미연동 → 회색 플레이스홀더로만 배치(비배선).
    /// - "동작하는 회색 초안"이 목표. 재실행 가능(idempotent).
    /// </summary>
    public static class UIPartyMenuPrefabBuilder
    {
        ...
        private static readonly Color WindowBg  = new Color(0.06f, 0.08f, 0.12f, 0.98f);
        private static readonly Color PanelBg   = new Color(0.09f, 0.12f, 0.16f, 1f);
        ...
        private static readonly Color AccentBtn = new Color(0.16f, 0.42f, 0.52f, 1f);
        ...
        [MenuItem("UPlayGround/UI/동료(파티) UI 프리팹 빌드 (초안)")]
        public static void Build()
        {
            ...
            try
            {
                ...
                Stretch(title);
                AddText(title, "동료", 34, TextMain, TextAlignmentOptions.Center);
                ...
                SetHeight(cardsRow, 460);
                AddHLG(cardsRow, spacing: 10, pad: 4).childForceExpandWidth = true;
                var battleEntries = new UIPartyBattleEntry[4];
                for (int i = 0; i < 4; i++)
                    battleEntries[i] = BuildBattleCard(cardsRow.transform, i);

                var emptyHint = NewUI("EmptySlotHint", center.transform);
                AddFlexible(emptyHint, 1);
                AddImage(emptyHint, new Color(0.10f, 0.13f, 0.17f, 1f), UISprite, sliced: true);
                var emptyText = AddText(NewUI("Text", emptyHint.transform), "+ 빈 슬롯", 22, TextSub, TextAlignmentOptions.Center);
                Stretch(emptyText.gameObject);
                ...
                var btnSave  = MakeButton("SaveButton",   bottom.transform, "저장",     out _, BtnBg);      SetWidth(btnSave.gameObject, 150);
        ...
        private static UIPartyMenuEntry BuildMenuEntryPrefab()
        {
            ...
            Stretch(selected);
            var selImg = AddImage(selected, new Color(Accent.r, Accent.g, Accent.b, 0.16f));
            ...
            Stretch(dimmed);
            var dimImg = AddImage(dimmed, new Color(0f, 0f, 0f, 0.55f));
        ...
        private static UIPartyBattleEntry BuildBattleCard(Transform parent, int index)
        {
            ...
            btn.targetGraphic = img;
            AddVLG(card, spacing: 4, pad: 6).childForceExpandHeight = false;

            var portrait = NewUI("Portrait", card.transform);
            AddFlexible(portrait, 1);
            var portImg = AddImage(portrait, new Color(0.12f, 0.15f, 0.19f, 1f));

            var name = AddText(NewUI("Name", card.transform), "이름", 20, TextMain, TextAlignmentOptions.Center);
            SetHeight(name.gameObject, 26);
            var level = AddText(NewUI("Level", card.transform), "Lv. 1", 16, TextSub, TextAlignmentOptions.Center);
            SetHeight(level.gameObject, 20);
            ...
            var hpText = AddText(NewUI("HpText", card.transform), "0 / 0", 15, TextMain, TextAlignmentOptions.Center);
            SetHeight(hpText.gameObject, 18);
            ...
            Stretch(selected);
            var selImg = AddImage(selected, new Color(Accent.r, Accent.g, Accent.b, 0.14f));
            selImg.raycastTarget = false;
            selected.AddComponent<LayoutElement>().ignoreLayout = true;
            selected.transform.SetAsFirstSibling();
            selected.SetActive(false);
        ...
        private static GameObject BuildDetailPanel(Transform parent, out UI_PartyDetailPanel comp)
        {
            ...
            AddVLG(detail, spacing: 6, pad: 14).childForceExpandHeight = false;

            // 상단: 초상 + 정보
            var top = NewUI("Top", detail.transform);
            AddFlexible(top, 1);
            AddHLG(top, spacing: 10, pad: 0);
            var portraitGo = NewUI("Portrait", top.transform);
            AddFlexibleW(portraitGo, 1f);
            var portrait = AddImage(portraitGo, new Color(0.12f, 0.15f, 0.19f, 1f));
            var info = NewUI("Info", top.transform);
            SetWidth(info, 210);
            AddVLG(info, spacing: 4, pad: 4).childForceExpandHeight = false;
            var name = AddText(NewUI("Name", info.transform), "이름", 28, TextMain, TextAlignmentOptions.Left); SetHeight(name.gameObject, 36);
            ...
            // 능력치
            AddText(NewUI("StatTitle", detail.transform), "능력치", 20, TextMain, TextAlignmentOptions.Left);
            var statBox = NewUI("Stats", detail.transform);
            AddImage(statBox, SlotBg, UISprite, sliced: true);
            ...
            // 역할 태그
            AddText(NewUI("RoleTitle", detail.transform), "역할/특성", 20, TextMain, TextAlignmentOptions.Left);
            var roleRow = NewUI("Roles", detail.transform);
            ...
            // 스킬 / 궁극기 (플레이스홀더, 비배선)
            AddText(NewUI("SkillTitle", detail.transform), "스킬", 20, TextMain, TextAlignmentOptions.Left);
            var skillRow = NewUI("Skills", detail.transform);
