...
**고정된 플레이어 시작점**에서 출발해, 수작업 검증된 N개 외곽 후보에서 보스 위치를 결정하고 중앙 보스를 별도 아레나에 준비한다. 외곽 위치는 작은 회색 신호로 안내하고, 중앙 위치는 외곽 3체 완료 후 큰 붉은 신호로 공개한다. 실제 보스의 정체는 조우 전까지 숨긴다.
...
외곽 보스는 즉시 활성화한다. 중앙 보스 오브젝트와 마커도 같은 레이아웃 단계에서 생성하되, 중앙 보스 오브젝트는 외곽 보스 세 체가 모두 처치될 때까지 비활성 상태로 유지한다. 저장 복원 시 외곽 처치 상태를 기준으로 중앙 활성 여부를 재구성한다.
...
- 중앙 평가는 시작 시 보스와 마커를 모두 숨기고, 외곽 보스 3체 처치 후 활성화하면서 큰 붉은 신호를 공개한다.
