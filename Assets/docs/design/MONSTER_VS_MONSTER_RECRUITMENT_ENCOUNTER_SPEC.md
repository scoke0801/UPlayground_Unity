...
LakeOfLife의 30~40분 단일 스토리 수직 슬라이스를 위해 영입 조우를 `화린 → 리안` 순서로 연결했다. 두 정의는 모두 `PersistUntilNewGame`을 사용하며 사이클 정산에 의존하지 않는다. 리안 조우의 선행 ID는 `test.combat.honoka_rescue`이므로 화린의 획득 후 대화까지 완료되기 전에는 진입 라우팅이 열리지 않는다.
...
| 2 | `test.combat.lian_rescue` | `DLG_Test_LianRescue` | `DLG_Test_LianPostRescue` | 거대한 흔적 조사 시 `DLG_Lake_DragTracks` |
...
리안 대표 프리팹은 기존 검증 구조와 같은 스켈레톤 근접·원거리 조합을 재사용하고, 캐릭터 원본 프리팹의 BT/GAS/MotionSet 연결을 유지한다. 참가자 `_recruitableAs`는 모두 `None`이며 파티 해금은 영입 서비스만 수행한다. 씬 배치는 첫 조우에서 약 32m 진행한 위치 `(1118.3672, 55, 423.17047)`다.
...
| 순서 | questId | 이름 | 목표 |
| --- | --- | --- | --- |
| 1 | `quest_sub_lake_missing_villagers` | 신전의 보물 | 안내인에게 신전으로 가는 길 확인 |
| 2 | `quest_sub_lake_rescue_honoka` | 숲길 안쪽으로 | 숲길 이동 → 붉은 천 조사 → 화린 구조 |
| 3 | `quest_sub_lake_rescue_lian` | 리안의 표식 | 리안 표식 조사 → 리안 구조 |
| 4 | `quest_sub_lake_follow_tracks` | 신전으로 가는 길 | 거대한 흔적 조사 → 묘령 대치 |
...
- 신전행의 마을 단계는 안내인 대화 플래그(`lake.story.guide_briefed`) 하나만 `NotifyStoryEvent`로 옮긴다. 미아와 조안의 플래그는 준 수색선에 속하며 `FLOW_LakeSearchQuestLine`의 노드·목표·선행 조건에서 제외한다.
- `MapReady`는 열기·복구·추적을 한 경로로 처리한다. 퀘스트가 아직 `Available`이면 열고, 이미 참인 안내인 플래그를 `CheckFlag → NotifyStoryEvent`로 다시 목표에 반영한 뒤 현재 활성 퀘스트를 추적한다. 플래그 변화 진입점은 저장 복원에서 다시 울리지 않으므로 이 재반영이 없으면 이미 대화를 끝낸 세이브에서 진행 불능이 된다.
- 준 수색은 미아 → 조안 → 동쪽 숲길로 이어지는 별도 보조 퀘스트다. 레거시 에셋 `quest_sub_hunter_skeleton_patrol`은 ID·GUID를 보존한 채 전용 현장 콘텐츠가 완성되기 전까지 `isContentEnabled: 0`을 유지하며, 화린·리안 구조 플래그를 목표로 사용하지 않는다.
...
| 리안 표식 | `clue_lian_marker` | 리안 조우 프리팹의 `MinimapMarkerRegistrar` |
| 리안 구조 | `encounter_lian_rescue` | 같음 |
| 거대한 흔적 | `clue_drag_tracks` | 같음 |
...
Play Mode 검증은 아직 수행하지 않았다. 확인할 것은 안내인 대화와 세 현장 단서의 순차 마커 노출, 안내인 대화 직후 다음 퀘스트 자동 수락과 추적 이동, 세 조우 완료 시 목표 반영, 각 단계 저장·로드 후 추적 복원이다. 미아·조안 대화가 신전행 퀘스트 진행도를 바꾸지 않는지도 함께 확인한다.
...
기존 화린·리안 공동 전투형은 수정하지 않고, 적대 결투형의 실제 저작 샘플로 묘령 조우를 추가했다. 이 조우는 LakeOfLife 구조선의 별도 확장 샘플이며 메인 플롯의 확정 사건을 바꾸지 않는다.

| 항목 | 값 |
| --- | --- |
| 조우 ID | `test.combat.siuha_duel` |
| 선행 조우 | `test.combat.lian_rescue` 완료 |
...
묘령은 무너진 길과 몰려든 괴물 때문에 신전 안쪽에서 사람이 죽는 일을 겪었고, 같은 일이 반복되지 않게 길을 막는다. 주인공 일행은 경고를 이해하지만 신전으로 가려는 목적을 포기하지 않아 충돌한다. 묘령은 체력이 소진되어 쓰러진 뒤에도 주인공이 공격을 멈춘 것을 보고 살의가 없었음을 확인하고, 막을 수 없다면 위험한 구간을 직접 안내하는 편이 낫다고 판단해 합류한다. 정체 확인 퀴즈나 “실력을 시험한다”는 명분은 사용하지 않는다.
...
### 21.15 현장 발견 중심 신전행과 주인공 대사 기능

신전행의 정보 전달 방식을 `NPC 설명 → 목적지 이동`에서 `단서 제시 → 이동·조사 → 현장 판단 → 위험 갱신`으로 바꿨다. 안내인은 신전으로 가는 길만 알려주고, 화린과 리안은 같은 목적지를 향하던 모험가로 현장에서 만난다. 준 수색은 미아와 조안이 맡는 별도 보조선이며 두 동료의 영입 조건이나 단서 해석에 관여하지 않는다.

```text
안내인 → 붉은 천 발견 → 화린 구조
  → 리안 표식 발견 → 리안 구조
  → 거대한 흔적 발견
  → 신전 길을 막는 묘령과 대치 → 묘령의 안내로 신전행 계속

미아 → 조안 → 동쪽 숲길의 준 일행 수색 (별도 보조 퀘스트)
```

- 신전행의 붉은 천·리안 표식·끌린 자국은 각각 3D 월드 비주얼, 조사 콜라이더, `FlowGraphInteractable`, `FlowGraphTriggerVolume`, `MinimapMarkerRegistrar`를 가진다. 붉은 표식은 짧은 말뚝에 묶은 천 조각으로 표현한다. 대화 힌트용 UI 이미지를 월드 SpriteRenderer로 재사용하지 않는다. 남색 천은 준 수색선의 단서 후보로만 보존하며, 신전행 프리팹에서는 비활성화하고 신전행 FlowGraph·퀘스트 목표와 연결하지 않는다.
...
- 세 단서는 `FlowGraphInteractable`의 트리거 후 비활성화 옵션을 사용한다. 진입점 발화가 실패하면 월드에 남아 재시도할 수 있고, 성공한 경우에만 해당 단서 GameObject를 비활성화해 중복 조사와 남은 상호작용 프롬프트를 제거한다.
- 상호작용 프롬프트는 해당 퀘스트가 `Active`이고 조사 플래그가 아직 거짓일 때만 표시한다. 거대한 흔적은 `quest_sub_lake_follow_tracks`가 활성화된 뒤에만 조사할 수 있으므로, 리안 구조 전에 현장을 지나가는 순서 이탈은 진행을 앞당기지 않는다.
...
주인공 대사는 질문 수가 아니라 **문장 기능**을 기준으로 다듬었다. 구조 장면에서는 부상을 관찰하고 동행 위험을 판단하며, 현장에서는 거대한 흔적의 방향으로 앞길의 위험을 갱신한다. 묘령 대치에서는 경고를 이해한 뒤에도 신전행을 계속하겠다는 결정을 직접 내린다. 장면마다 주인공이 `관찰 → 판단 → 행동 결정` 중 최소 두 단계를 담당하게 하고, NPC 설명을 꺼내기 위한 짧은 질문의 연쇄는 피한다.

화린·리안·묘령은 모두 주인공과 초면이라는 관계 전제를 대사에 반영한다. 화린 구조 직후에는 목적지가 같은 임시 동행으로 시작하고, 리안의 이름은 화린에게 처음 듣는다. 리안 구조 장면에서는 화린이 주인공에게 도움받았다고 말해 신뢰를 보증하며, 이후 세 사람은 역할을 나눠 신전 길의 위험을 살핀다. 묘령은 위험을 아는 사람으로서 낯선 무장 일행을 돌려보내려 하고, 주인공은 목적을 포기하지 않아 충돌한다. 전투 뒤 묘령은 막는 대신 앞장서는 선택을 한다. 관계 단계는 `낯선 구조자 → 목적지가 같은 임시 동행 → 제3자의 신뢰 보증 → 공동 이동 → 위험을 둘러싼 충돌 → 길잡이 합류` 순서를 따른다.
...
### 21.16 신전의 보물 오프닝과 동행 리듬

LakeOfLife 오프닝의 주인공은 **원하는 것을 얻게 해준다는 신전의 보물 소문을 직접 확인하러 온 외지인**이다. 주인공이 실제로 원하는 것은 아직 밝히지 않는다. 플레이어는 보물의 실재 여부와 주인공의 소망이라는 두 질문을 품고 출발하며, 화린과 리안은 같은 목적지를 향하던 모험가로 신전 길에서 만난다. 미아와 조안의 준 수색은 이 여정과 독립된 보조선이다.

```text
신전의 보물 소문을 확인하러 호숫가 신전으로 향한다
  → 안내인에게 길과 숲의 위험을 듣는다
  → 숲길의 붉은 천을 확인하고 몬스터에게 포위된 화린을 돕는다
  → 화린과 목적지가 겹쳐 임시 동행한다
  → 붉은 천 표식을 직접 조사한다
  → 화린이 앞의 위험을 판단하고 주인공이 전투를 결정한다
  → 리안을 구조한다
  → 세 사람이 신전 길의 위험 흔적을 확인한다
  → 길을 막는 묘령과 충돌한 뒤 함께 신전으로 향한다
```

- 새 게임 오프닝은 일반 대화창을 사용하지 않는다. 선택 주인공의 뒷모습 삽화 위에 이름표·초상화·진행 아이콘 없는 시네마틱 자막을 순차 노출한다.
- 첫 자막은 `호숫가의 오래된 신전에, 원하는 것을 얻게 해주는 보물이 있다고 한다.`, 두 번째 자막은 `……직접 확인해 볼까.`다. 이어서 별도 지역 타이틀 스타일로 `생명의 호수`를 노출한다.
- 지역 타이틀 뒤에는 `목표: 호숫가의 신전으로 향하라`를 짧게 노출한다. 삽화에는 14.5초의 느린 팬·푸시인을 적용하고 마지막 0.7초에 실제 플레이 화면으로 디졸브한다. 오프닝 종료 후 퀘스트 `신전의 보물`을 시작한다.
- 네 연속 퀘스트의 ID·GUID는 보존한다. 첫 퀘스트는 안내인 대화만 요구하고, 미아·조안 플래그와 화린·리안 구조선의 결합은 제거했다. `FLOW_LakeSearchQuestLine`은 안내인 플래그와 현재 활성 신전행 퀘스트만 복원·추적한다.
- 보물은 초반의 추진력이지 즉시 확정되는 사실이 아니다. 오프닝과 퀘스트 문구는 모두 `소문`과 `직접 확인`의 수준을 넘지 않는다.
- 화린 구조 후 대사는 감사·리안과 흩어진 상황·표식 수색만 남긴다. 주인공은 별도의 구조 약속을 하지 않고 호수로 가는 길이 같다는 사실만 밝힌다.
- 리안의 표식은 붉은 천을 사용하되, 긴 매듭 끝이 프리팹 로컬 `+Z`, 즉 리안 조우 방향을 실제로 가리키도록 해 앞선 단서와 형태로 구분한다. 조사 삽화도 같은 붉은 천 이미지를 재사용한다.
- 리안 조우 진입 시 `lake.story.lian_danger_spotted`를 확인해 짧은 전투 전 대화를 한 번만 재생한다. 화린이 위험과 경로를 판단하고, 주인공이 우회 여부와 전투 결정을 맡은 뒤 적 그룹을 활성화한다.
- 리안 구조 뒤에는 화린이 주인공의 도움을 짧게 보증하고, 주인공은 부상과 이동 가능 여부만 확인한다. 재치 있는 초면 농담이나 자기 성격을 과시하는 대사는 두지 않는다.
- 거대한 흔적 대화는 `위험 발견 → 신전 방향 확인` 두 줄로 줄였다. 이미 화면에 보이는 자국을 되풀이해 설명하거나 준 일행의 흔적으로 단정하지 않는다.

#### 정보 공개 순서 QA

| 장면 | 장면 시작 시 플레이어가 아는 것 | 이 장면에서 처음 공개할 정보 | 미리 노출하면 안 되는 것 |
| --- | --- | --- | --- |
| 오프닝·안내인 | 신전에 보물이 있다는 소문 | 신전 방향, 숲길의 위험 | 화린·리안·묘령의 이름과 사정 |
| 첫 붉은 천 | 누군가 일부러 천을 남겼음 | 의도적으로 놓인 단서라는 관찰 | 천의 주인, 표식 규칙, 리안의 이름 |
| 화린 구조 | 구조 대상은 초면임 | 화린의 이름 → 동행자가 있었음 → 리안의 이름 → 붉은 천 약속 → 같은 신전 목적지 | 리안이 실제로 어느 방향에 있다는 확정 |
| 리안 표식 | 리안이 붉은 천을 쓸 가능성이 있음 | 발견한 붉은 천이 리안의 것이라는 화린의 확인 | 리안의 안전 여부 |
| 리안 구조 | 리안이 앞에서 포위됐음 | 구조 뒤 신전까지 남은 거리와 동행 역할 | 준 일행과의 연관 |
| 거대한 흔적 | 세 사람이 신전으로 이동 중임 | 신전 방향에 큰 위험이 있다는 현장 판단 | 흔적의 주체를 근거 없이 확정하는 말 |
| 묘령 대치 | 낯선 인물이 길을 막음 | 묘령의 이름, 붕괴된 길과 괴물 위험, 길을 막는 동기 | 합류 의사 |
| 묘령 전투 뒤 | 묘령이 살의 없이 제압됨 | 길잡이로 앞장서겠다는 선택 | 전투 전부터 동료인 듯한 호칭·태도 |

화자 이름표도 같은 공개 순서를 따른다. 현재 UI는 `speakerId`를 표시명으로 바로 사용하므로 화린과 묘령은 각각 첫 발화에서 스스로 이름을 밝힌다. 퀘스트명·목표·합류 알림이 자기소개보다 먼저 이름을 노출하지 않는지는 Play Mode에서 확인한다.

대사 어투는 다음 Voice Profile을 기준으로 통일한다. 주인공은 짧게 관찰하고 결정을 내리며, 정보를 꺼내기 위한 억지 질문은 하지 않는다. 화린은 급한 상황에서도 핵심을 또렷하게 말하고 리안과 관련된 문장은 짧아진다. 리안은 길과 위험을 먼저 살피는 차분한 말투를 쓴다. 묘령은 전투 전에는 단호한 명령형을 쓰되 과장된 위협은 피하고, 전투 뒤에는 경계가 누그러져도 갑자기 친근해지지 않는다.

오프닝 삽화는 기존 Main 대화 재생 흐름을 유지하되 `CinematicNarration` 표현 모드를 사용한다. 이 모드는 일반 대화 패널과 재생 컨트롤을 숨기고, 전용 하단 자막·지역 타이틀·여러 줄 간 삽화 유지·느린 팬·푸시인·플레이 화면 디졸브만 담당한다. 실제 길이, 자막·타이틀 안전 영역, 전환 시 프레임, 단계별 저장·로드는 Play Mode에서 확인해야 한다.
