...
구현 위치: `Assets/02.Scripts/Gameplay/TriggerSystem/`

- `TriggerComposer`, `TriggerContext`, `TriggerRepeatPolicy` 추가
- `TriggerSourceSO`, `TriggerConditionSO`, `TriggerActionSO` 베이스 추가
- Collider Enter/Exit, Group Defeated Source 추가
- Global Flag, Story Progress, Quest Status, Actor/Group Alive, Random Chance, AND/OR/NOT Condition 추가
- Quest, Story, Global Flag, Group Activate, Camera Snapshot, UnityEvent, Delay, Sequence Action 추가
- 씬 오브젝트 참조용 `TriggerSceneReferences` 추가

### Phase 2~4 — 이식 및 레거시 제거

- 기존 `QuestTriggerZone`, `StoryTriggerZone`, `GroupSpawnTrigger`, `GroupStoryTrigger`, `CameraSnapshotSequenceTrigger` 사용처를 `TriggerComposer` 구성으로 변환
- 변환된 Source/Action 에셋은 `Assets/10.Datas/TriggerSystem/Migrated/` 아래에 생성
- 비활성 no-op `GroupStoryTrigger` 컴포넌트는 동작이 없으므로 제거
- 레거시 트리거 스크립트 5종 삭제 완료
