...
namespace UPlayGround.UI
{
    ...
    /// </summary>
    public class UI_Scene_CraftMenu : UI_SceneBase
    {
        ...
        [SerializeField] private UICraftingRecipeSlot  _recipeSlotPrefab;
        ...
        [SerializeField] private UICraftingIngredientSlot _ingredientSlotPrefab;
        ...
        // ──── 런타임 상태 ────
        private readonly List<UICraftingRecipeSlot>      _spawnedRecipeSlots     = new List<UICraftingRecipeSlot>();
        private readonly List<UICraftingIngredientSlot>  _spawnedIngredientSlots = new List<UICraftingIngredientSlot>();
        ...
        protected override void Update()
        {
            ...
            if (RecipeMgr.IsCrafting())
                SetProgress(RecipeMgr.GetCraftingProgress());
        }
        ...
        private void RefreshRecipeList()
        {
            ...
            if (!RecipeMgr.IsDBLoaded) return;

            var ids = RecipeMgr.GetUnlockedRecipeIDs();

            bool selectionStillVisible = false;

            foreach (var id in ids)
            {
                var data = RecipeMgr.GetRecipeData(id);
        ...
        private void RebuildNavigation()
        {
            ...
            foreach (UICraftingRecipeSlot slot in _spawnedRecipeSlots)
            {
        ...
        {
            var recipe = RecipeMgr.GetRecipeData(recipeID);
        ...
        private void SetQuantityMax()
        {
            ...
            if (RecipeMgr.IsCrafting()) return;

            int max     = RecipeMgr.GetMaxCraftableQuantity(_selectedRecipeID);
        ...
        private void OnClickCraft()
        {
            ...
            if (RecipeMgr.IsCrafting())
            {
                RecipeMgr.CancelCrafting();
                return;
            }

            RecipeMgr.TryStartCrafting(_selectedRecipeID, _quantity);
        }
        ...
        private void RefreshCraftButton()
        {
            ...
            bool canCraft = RecipeMgr.CanCraft(_selectedRecipeID, _quantity);
            CraftAvailabilityReason reason = RecipeMgr.GetCraftAvailabilityReason(_selectedRecipeID, _quantity);
