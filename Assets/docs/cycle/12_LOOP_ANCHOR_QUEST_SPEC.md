...
주민 모델을 확정하지 않은 상태에서 안내인에게 분실물 역할까지 겸임시키지 않는다. 위 네 데이터는 P0 에셋에 배정되었으며, 이후 변경할 때도 주민과 안내인의 기억 역할을 합치지 않는다.
...
|---|---|---|---|---|
| 1 | `obj_anchor_request` | StoryEvent | `cycle.anchor.request_started` | `미아에게 무슨 일이 있었는지 듣는다` |
...
+ 분실물 보유
→ "잠깐. 내 리본을… 내가 잃어버린 줄도 몰랐는데." 계열 반응
→ DeliverItemToQuest
→ first_return_anchor_completed=true
```

선회수 반응은 물건을 보자마자 알아본 미아의 놀람으로 표현하고, 다른 경로에서도 성립해야 하는 후속 대사에는 `처음부터 알고 있었다`는 단정을 쓰지 않는다.

### 9.3 첫 귀환 — 먼저 대화

선택지:

- `[기억] 나무 울타리 옆을 먼저 볼게.` — `lostitem_resolved_once=true`일 때만 표시
- 평범하게 부탁을 듣는다
...
초회차 리본 반환 전에는 전투 목표를 먼저 말하지 않는다. `cycle.anchor.lostitem_resolved_once=false`이면 미아가 울타리 쪽을 살핀다는 한 줄만 보여 주고, 반환 뒤에만 세 갈래와 세 사람 안내로 넘어간다. 첫 귀환이 시작됐지만 리본을 다시 돌려주기 전에도 같은 안내를 재생하지 않고, 미아를 먼저 만나라는 짧은 대기 대사만 보여 준다.

대화의 기능:

1. 세 경로 중 하나를 사실과 다르게 단정하지 않고, 같은 풀숲과 서로 다른 기억을 짚는다.
...
P0 구현에는 다음 네 콘텐츠 값이 확정되어 반영되었다.

1. 주민: `Npc_Mia`, 미아, npcId `210101`
2. 분실물: 파란 리본, itemId `250101`, 동적 오브젝트 `CycleAnchor_BlueRibbon`
3. 위치: 마을 입구 나무 울타리 옆 풀숲, 안내인은 주민 바로 옆이 아닌 가까운 조용한 지점
4. 문안: `DLG_Npc_Mia_CycleAnchor`, `DLG_Npc_Guide`의 2026-08-15 정적 검토본

이후 문안을 바꿀 때도 선회수·기억 선택·평범한 재해결 세 경로가 모두 사실에 맞게 같은 안내인 완료 이벤트로 합류해야 한다.
