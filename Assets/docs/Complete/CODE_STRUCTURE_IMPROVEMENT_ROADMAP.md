...
CLAUDE.md가 현재 코드와 어긋나 있다. Claude Code 협업 구조에서는 CLAUDE.md가 틀리면 매 세션 잘못된 전제로 시작하므로 주기적 재동기화 가치가 크다.
...
`.Instance` 직접 접근 1,208회를 DI로 전면 전환하는 것은 회수되는 이득 대비 변경 위험이 너무 크다. **핫스팟만** 정리한다.
