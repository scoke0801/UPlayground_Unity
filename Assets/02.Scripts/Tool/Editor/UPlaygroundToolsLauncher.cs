...
namespace UPlayGround.Editor
{
    public class UPlaygroundToolsLauncher : EditorWindow
    {
        ...
        private static readonly (string Category, ToolEntry[] Tools)[] s_categories =
        {
            ...
            ("콘텐츠 / 퀘스트", new[]
            {
                Tool("퀘스트 목표 배선 검증", "UPlayGround/콘텐츠/퀘스트/퀘스트 목표 배선 검증", "목표의 완료 신호와 마커 지점이 실제로 공급되는지 검사합니다.", "위치·서사 이벤트 ID를 알려주는 트리거·대화 액션·흐름 그래프가 있는지, 마커 지점이 NPC·조우·씬 어딘가에 있는지, 선행 목표와 선행 퀘스트가 존재하는지 검사합니다. 오타 하나로 목표가 영원히 완료되지 않는 상태를 저작 단계에서 찾습니다."),
            }),
            ("콘텐츠 / 도감", new[]
            {
