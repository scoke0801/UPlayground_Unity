...
> 작성일: **2026-08-12**<br>
> 상태: **구현 승인 대기 / 대사 JSON·QuestSO·FlowGraph 에셋 미착수**<br>
...
현재 확인된 시작 마을 전용 NPC 데이터는 `NPC_Story_Guide`뿐이다. 반복을 기억하지 못하는 주민과 기억하는 안내인은 같은 인물로 합치면 안 되므로 다음 데이터가 추가로 필요하다.

| 필요 데이터 | P0 결정 |
|---|---|
| 분실물 주인 주민 | 안내인과 다른 NPC Actor/프리팹을 하나 지정 또는 저작 |
| 분실물 | 중복되지 않는 신규 Story Item ID와 표시명·아이콘 지정 |
| 초회차 위치 | 시작 NPC에서 짧게 보이거나 20~30초 내 도달 가능한 지점 |
| 첫 귀환 위치 | 초회차와 같은 위치 |
...
| 종류 | ID |
|---|---|
| 앵커 반복 퀘스트 | `quest_story_lost_item_anchor` |
| 첫 요청 들음 이벤트 | `story.anchor.lostitem.request_heard` |
| 물건 반환 이벤트 | `story.anchor.lostitem.returned` |
| 첫 귀환 도착 이벤트 | `story.first_return.arrived` |
| 첫 귀환 안내인 대화 이벤트 | `story.first_return.guide_talked` |
...
| `cycle.story.first_return_anchor_completed` | 첫 귀환 분실물 반환·주민 반응 완료 |
| `cycle.story.first_return_guide_completed` | 첫 귀환 안내인 대화 완료 |

모두 새 게임 단위 영구 플래그다. `first_return_request_heard`는 P0 첫 귀환 분기에서만 쓰며 이후 일반 반복 앵커로 재사용하지 않는다.

### 3.3 데이터 승인 때 배정할 ID

- 주민 `actorId`와 Quest `npcId`
- 분실물 ItemSO의 숫자 `itemId`
- 동적 오브젝트 spawn ID
- 안내인 대화 지점 location ID
...
```text
questId: quest_story_lost_item_anchor
questName: 잃어버린 물건
questType: Sub
...
|---|---|---|---|---|
| 1 | `obj_anchor_request` | StoryEvent | `story.anchor.lostitem.request_heard` | `마을 주민의 부탁을 들어본다` |
| 2 | `obj_anchor_collect` | ItemCollect | 신규 분실물 itemId | `잃어버린 물건을 찾는다` |
| 3 | `obj_anchor_deliver` | ItemDeliver | 주민 npcId + itemId | `물건을 주민에게 돌려준다` |
...
|---|---|---|---|---|
| 1 | `obj_first_return_arrived` | StoryEvent | `story.first_return.arrived` | `귀환 포털로 마을에 돌아간다` |
| 2 | `obj_first_return_anchor` | StoryEvent | `story.anchor.lostitem.returned` | `다시 일어난 부탁을 해결한다` |
| 3 | `obj_first_return_guide` | StoryEvent | `story.first_return.guide_talked` | `안내인과 이야기한다` |
...
`quest_main_001.autoAcceptOnNewGame`은 기존대로 유지한다. 대신 앵커 Flow가 `quest_story_lost_item_anchor`를 HUD 추적 대상으로 명시하고, 완료 뒤 `quest_main_001`로 되돌린다. 생활 튜토리얼 중에는 보스 목표가 추적되지 않고, 보스 마커도 실제 사이클 시작 전까지 생성되지 않는다. 초회차 앵커 완료 시 main001이 아직 Active가 아니면 한 번 수락을 보정한 뒤 추적한다.

## 8. 동적 분실물 오브젝트

P0 전용 `CycleLoopAnchorSpawner` 또는 동등한 씬 컴포넌트를 둔다. 범용 월드 리셋 매니저로 확장하지 않는다.

### 생성 조건

```text
quest_story_lost_item_anchor가 Active
AND 인벤토리에 분실물 0개
AND obj_anchor_collect 미완료
AND (
...
획득 시 일반 인벤토리 API로 1개를 지급하여 ItemCollect objective를 갱신한다. 반환은 `InventoryManager.DeliverItemToQuest` 경로를 사용해 아이템 소비와 ItemDeliver objective 갱신을 한 번에 처리한다.
...
- `QuestStatusDialogueConditionSO`: 문자열 questId의 상태 비교

기존 `FlagConditionSO`를 함께 사용한다. 분기 결과는 다음과 같다.
...
`cycle.story.first_return_anchor_completed=true`가 된 뒤 안내인 기본 대화 그래프 최상단에 해당 분기를 추가한다. 진행도만으로 분기하면 포털 복귀 전에 대화가 소진될 수 있으므로 반드시 플래그를 함께 확인한다.
...
대화 마지막 Event 노드에서 `cycle.story.first_return_guide_completed=true`를 설정한다. 대화 도중 중단되면 마지막 이벤트가 실행되지 않으므로 다시 상호작용해 완료할 수 있다.
...
| `first_return_anchor_resolved` | OnFlagChanged | NotifyStoryEvent(request_heard, 멱등) → CompleteQuest(anchor) → NotifyStoryEvent(anchor.returned) |
...
- 첫 귀환 포털 정산 직후 자동 저장하고, 앵커 반환과 안내인 완료 뒤에도 즉시 저장한다.
